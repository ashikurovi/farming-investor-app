...
export default function InvestmentDetailPage() {
  ...
  return (
    <main className="min-h-screen space-y-6  p-4 sm:p-8">
      {/* Header */}
            ...
            <div className="p-6 sm:p-8">
                ...
                {(() => {
                  ...
                  return (
                    <div className="inline-flex items-center gap-2 rounded-2xl bg-zinc-50 px-4 py-2 ring-1 ring-zinc-200">
                      ...
                      />
                      <span
                        className={`text-xs font-bold uppercase tracking-wider ${isExpired ? "text-red-700" : effectiveActive ? "text-primary" : "text-zinc-500"}`}
                      >
                        {isExpired
                          ? "Expired"
                          : effectiveActive
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {[
                  ...
                  <div
                    key={label}
                    className="flex items-center justify-between rounded-2xl border border-zinc-100 bg-zinc-50/50 p-4 transition hover:bg-white hover:shadow-sm"
                  >
            ...
            {investment.deeds && investment.deeds.length > 0 ? (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {investment.deeds.map((deed) => (
                  <div
                    key={deed.id}
                    className="group relative overflow-hidden rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:ring-2 hover:ring-[color:rgba(77,140,30,0.22)]"
                  >
